...
講義で使う主な部品は，`mmexample`，`mmproblem`，`mmsolution`，`\MMAlternative`，`\MMNote` である．`\MMNote` は従来どおり行頭に右向きの三角を置くが，三角と注記本文は本文と同じ基準文字サイズで組み，注記本文を `\small` などで縮小しない．注記本文の書体とウェイトも本文と同じにする．この右向きの三角は講義の短い注記だけを意味し，確認系の拡張記号（§9）の上向き三角形とは別物である．
...
確認系の記事では，上の正方形・矢印に加えて，第 9 章の拡張記号（要点，注意，
方針，公式，検算，発展，図の指示）も選んで使える．拡張記号は確認系のための
部品であり，講義やベーシック演習へ持ち込まない．
...
| 確認 | 要点，注意，方針，公式，検算，発展，図の指示 | §9.2 の `\MMConfirmationGeneralX` ／ `\MMConfirmationAdvancedX` |

拡張記号の語幹 `X` は `Point`，`Caution`，`Hint`，`Formula`，`Check`，`Extension`，`Figure` の 7 種である．形と意味の対応は §9.1 に定める．
...
| 答の終わり | `\MMQED` |
| 本文中の参照 | `\MMSeeAlso` |
| 難易度 | `\MMDifficultyDots` |
| ラベル | `\MMBlackLabel` ／ `\MMOutlineLabel` ／ `\MMTintLabel` |
| 数値記号 | `\MMMockNumber` ／ `\MMOutlineNumber` ／ `\MMSquareNumber` ／ `\MMFilledSquareNumber` |
...
- 三角注記（`\MMNote`，右向きの三角）の本文だけを `\small` などで縮小したり，別書体へ切り替えたりしない．
...
- 強調のために本文の書体，ウェイト，文字サイズを変えない．圏点，用語のゴシック，ルビから選ぶ（§11.4）．
- 圏点の記号（`\MMKentenMark`）を記事ごとに差し替えない．誌面で 1 種類に統一する．
- 拡張記号（§9）を確認系以外の記事へ持ち込まない．また，既に埋まっている形へ新しい意味を割り当てない．
...
正方形は「コメント」，「矢印＋注」は「注釈」，そして講義の `\MMNote` が
行頭に置く右向きの三角は「講義の短い注記」を意味しており，この三つの意味は
既に埋まっている．したがって新しい意味には，必ず新しい形を割り当てる．
一方，斜線入りと黒塗りの二値はこの体系の文法であり，形によらず同じ意味を
持つ．すなわち，どの形についても，斜線入りはすべての読者に向けたもの，
黒塗りは意欲的な読者に向けたものを表す．

拡張記号の三角形（`Point`）は，`\MMNote` の三角とは向き・用途・記事種別で
区別する．`Point` は上向きの正三角形で，確認系の記事の中でだけ使い，
斜線入りと黒塗りの対を持つ．`\MMNote` の三角は右向きで，講義の記事の
行頭にだけ置き，読者区分の二値を持たない（§3.1）．両者を同じ紙面へ
混在させない．
...
| 正方形 | コメント | `Comment`（既存） |
| 矢印＋注 | 注釈 | `Note`（既存） |
| 右向き三角形 | 講義の短い注記 | `\MMNote`（既存，読者区分なし） |
| 上向き三角形 | 要点 | `Point` |
| 菱形 | 注意，誤りやすい点 | `Caution` |
| 六角形 | 方針，着眼 | `Hint` |
| 円 | 定義，定理，公式 | `Formula` |
| 二重丸 | 検算，確かめ | `Check` |
| 五角形 | 発展，大学範囲 | `Extension` |
| 扇形 | 図を見よ | `Figure` |

「発展」の形についても斜線入りと黒塗りの両方を用意してある．教科書の範囲を
超えるが全読者に読んでほしい補足には斜線入りを，意欲的な読者だけに向けた
発展には黒塗りを使う．塗りは対象読者だけで決め，形の意味とは独立に扱う．

### 9.2 命令と環境

語幹 `X`（`x` はその小文字）について，既存の正方形・矢印とまったく同じ
構成の命令と環境が定義される．

| 種類 | 名前 |
|---|---|
| マーク単体（全読者） | `\MMConfirmationStripedXMark` |
| マーク単体（意欲的な読者） | `\MMConfirmationFilledXMark` |
| 短いコメント（全読者） | `\MMConfirmationGeneralX[番号]{本文}` |
| 短いコメント（意欲的な読者） | `\MMConfirmationAdvancedX[番号]{本文}` |
| 長いコメント（全読者） | `mmconfirmationgeneralx` |
| 長いコメント（意欲的な読者） | `mmconfirmationadvancedx` |

```latex
\MMConfirmationGeneralCaution{和から一般項を求めたときは，$n=1$ を必ず別に確かめる．}

\MMConfirmationAdvancedExtension{同じ考え方は，母関数を用いた議論へ延長できる．}

\begin{mmconfirmationgeneralhint}
まず差を作る．
\[
  \sum_{k=1}^{n}(b_{k+1}-b_k)=b_{n+1}-b_1
\]
...
番号の付け方は，§3.3 の注記番号と同じ規則に従う．関連する系列が続くときだけ
番号を付け，単発の記号には付けない．

### 9.3 描画の規約

拡張記号も，正方形と矢印と同じ共通描画部品を通す．したがって，すべての記号で
輪郭線幅，斜線の角度，間隔，線幅，開始位置，ベースライン合わせが一致する．
高さはすべて 0.82 em で，形が違うので幅だけが異なる．斜線の走査範囲は記号幅から
決まるため，幅の広い記号でも斜線が途切れず，位相もずれない．記号の外接矩形の
幾何学的中心を，本文第 1 行の字面中央に揃える．

原稿側で Unicode 記号，罫線文字，TikZ の汎用パターンなどを用いて，これらに似た
記号を作り足さない．汎用パターンはページ座標に固定されるため，同じ記号が
出現位置によって別の位相で刷られてしまう．

「図を見よ」に台形ではなく扇形を割り当てているのは，本文の大きさでは台形が
正方形と紛らわしく，正方形の意味が予約されているためである．新しい形を足す
ときも，本文の大きさで正方形および既存の形と見分けられることを必ず確かめる．

### 9.4 ラベル，数値記号，難易度

| 命令 | 用途 |
|---|---|
| `\MMBlackLabel{}` | 黒地のラベル（既存） |
| `\MMOutlineLabel{}` | 枠だけのラベル |
| `\MMTintLabel{}` | 淡い地のラベル |
| `\MMMockNumber{}` | 黒丸数字（既存） |
| `\MMOutlineNumber{}` | 白丸数字 |
| `\MMSquareNumber{}` | 角丸四角数字（白地） |
| `\MMFilledSquareNumber{}` | 角丸四角数字（黒地） |
| `\MMDifficultyDots{m}{n}` | 難易度を `n` 段階中 `m` で示す |
| `\MMQED` | 答の終わり |
| `\MMSeeAlso{}` | 本文中の参照 |

ラベルは，黒地，枠だけ，淡い地の順に弱くなる．同じ紙面に複数のラベルが
並んで黒い塊になるときに，二番目，三番目として使う．

角丸四角数字は必ず数字を入れて使い，0.82 em のインライン記号として使わない．
これは番号の器であり，コメントを表す正方形とは大きさも役割も別である．
読者が書き込む学習チェック欄には，新しい四角を作らず `\MMBlank` を使う．

`\MMDifficultyDots` は難易度だけを表す．目標時間は従来どおり `\MMDifficulty`
と `\MMTimeMarks` で表し，両者を混同しない．ドットは記号と同じベースライン
規則で組むため，同じ行に置いた記号と高さがそろう．

`\MMQED` は答の最終行の右端に「二重斜線」を置く．正方形は白抜きでも黒塗りでも
コメントを意味するため，答の終わりに四角を使わない．

### 9.5 避けること

- 新しい意味へ正方形，矢印または `\MMNote` の右向き三角を割り当てない．
- 拡張記号の一つの形に，斜線入りと黒塗りの片方だけを用意しない．講義の
  `\MMNote` は読者区分を持たない既存の部品であり，この規則の対象外である．
- 塗りを装飾上の好みで選ばない．塗りは対象読者だけで決める．
- 拡張記号を原稿側で描き直し，斜線の位相をばらつかせない．
- 角丸四角数字を，数字を入れずに記号として使わない．
- 答の終わりや学習チェック欄に，新しい四角を作らない．
...
`columnrule` は，段と段の間に 0.25 pt の細い罫を `SMrule` の色で引く．罫の長さは，左右の段のうち長いほうの内容の高さに合わせる．したがって記事の最終ページのように段が短いページでは罫も短くなり，本文の無いところへ垂れ下がらない．全段幅ブロック（§11.7）の上では，罫はブロックまで届く．なお，段を版面いっぱいへ伸ばす LaTeX 標準の `\flushbottom` を使う文書では，罫も版面の高さいっぱいに引かれる．
